...
- do uzyskania dostępu do filtrowanych treści WWW, dysydenci w różnych reżimach mogą wejść na zakazane przez te reżimy strony i usługi - [Sekurak "W cieniu rosyjskiej cenzury… Twitter dostępny jest przez Tora"](https://sekurak.pl/w-cieniu-rosyjskiej-cenzury-twitter-dostepny-jest-przez-tora/)
...
Hasła są najstarszym istniejącym systemem uwierzytelnienia (ang. autentication) użytkownika, który wciąż pomimo wszystkich wad jest używany. Dlaczego? Bo jest prosty. Jest przy tym łatwy do ataku i bardzo zawodny.
...
2. Co się wtedy wydarza? Akcja na przycisku, albo też zwykły Enter uruchamiają działanie formularza, który wysyła dane (login i hasło) do weryfikacji.
3. Mechanizm weryfikacji może mieć różne formy ale zasadniczo jest to proces porównujący otrzymany zestaw danych z zapisem wszystkich istniejących w systemie kont z ich hasłami.
   ...
   - kiedy odnaleziony jest identyczny, porównuje się odebrane hasło z tym przypisanym do danego konta.
...
Hasła muszą być trzymane z postaci zaszyfrowanej. To niestety nie zawsze się zdarza. Co do zasady: nikt poza użytkownikiem nie może znać hasła. Ale nie chodzi tu tylko o mało prawdopodobną sytuację, kiedy to admin loguje się użytkownikiem i wyczynia na jego notabene konto niecne brewerie. Nikt normalny nie ma na to czasu. Wiadomo, że hasła są, albo przynajmniej powinny być najbardziej strzeżoną częścią systemu. Nie ma jednak systemu odpornego na atak, nie można nawet zakładać, że takowy istnieje. Trzeba brać pod uwagę prawdopodobieństwo, że wcześniej czy później każda baza z danymi użytkowników wpadnie w ręce bandytów, którzy z takich czy innych powodów ją upublicznią. Co wtedy?

- Jeżeli hasła są trzymane w postaci jawnej system jest w całości przejęty. To oznacza, lub powinno oznaczać kompletną kompromitację i bankructwo organizacji mającej ten system.
- Sytuacja jest zupełnie inna jeśli hasła są zaszyfrowane. Bandyci wciąż nie są bez szans. Mogą wykupić moc obliczeniową w chmurze i hashcatem przelecieć bazę haseł. Jeżeli twórcy systemu pomyśleli, ale nie do końca i zastosowali jakiś prosty algorytm szyfrujący typu MD5, a użytkownicy stosowali hasła typu '12345sierpien' (sierpien, bo co miesiąc muszą zmieniać hasło), to bardzo szybko i bardzo tanio bandyci osiągnęli ten sam pozytywny dla siebie rezultat.
- Zgoła inaczej jest jeżeli twórcy systemu pomyśleli poprawnie i wdrożyli właściwy algorytm szyfrujący czyli bcrypt oraz założyli dobre limity na liczbę znaków, czyli minimum 15 znaków, a maksimum na tyle ile nie będzie problemem dla maszyny (bo szyfrowanie jest zasobożerne).
...
Tu jeszcze jedna rzecz wymaga wyjaśnienia: załóżmy, że deweloperzy stworzyli poprawny system, zastosowali szyfrowanie trzymanych w bazie haseł bcryptem. Przy logowaniu użytkownik wpisuje hasło w postaci jawnej. Jak zatem system porównuje otrzymaną z logowania jawną frazę hasła z zaszyfrowanym hasłem z bazy? To proste: hasło otrzymane od logującego się użytkownika szyfruje tym samym szyfrem i dopiero wtedy je porównuje. System również nie zna jawnego hasła. Bcrypt celowo jest tak zaprojektowany, żeby zabierać jak najwięcej mocy obliczeniowej. 
...
- muszą być unikalne
- powinny być trudne kryptograficznie
