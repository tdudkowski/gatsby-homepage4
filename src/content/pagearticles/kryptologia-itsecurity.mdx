...
### Odcinek 01 - początek [2022-02-14]
...
### Odcinek 02 - menedżer haseł [2022-02-28]
...
### Odcinek 03 - VPN, Tor i komunikatory [2022-03-14]
...
Zwykły użytkownik może potrzebować VPN głównie dla fałszowania lokalizacji. Zależnie bowiem od tego skąd się łączy do internetu, jakie miasto, państwo wskazuje jego IP może otrzymać ofertę podróży lub noclegu w innych cenach. Podobnie jest w przypadku usług VoD typu Netflix: chcemy korzystać z amerykańskiej oferty Netflixa? Musimy mieć IP z USA. To uzasadnia kupno usługi VPN, tyle, że nie ma nic wspólnego z bezpieczeństwem.

Chcemy anonimowości (wiadomo co, ale nie wiadomo kto)? Od tego jest Tor.

- ArubaCloud PL ["ArubaCloud- Webinar - Wszystko co chcecie wiedzieć o VPN" [YT 1:32:44]](https://www.youtube.com/watch?v=Sq6RpgXNDmU)
...
Zacznijmy jednak od tego jak to w ogóle działa. Mamy formularz logowania. Wpisujemy nazwę użytkownika, hasło, potem klikamy button. Co się wtedy dzieje? Akcja na przycisku, albo też zwykły Enter uruchamiają działanie formularza, który wysyła obie te dane (login i hasło) do weryfikacji. Mechanizmem weryfikacji jest zapis wszystkich istniejących w systemie kont z ich hasłami. Wpisany login porównywany jest z wszystkimi nazwami kont i kiedy odnaleziona jest identyczna, porównuje się odebrane hasło z tym przypisanym do danego konta. Jeżeli login istnieje i hasło zgadza się z tym zapisanym w systemie do programu odsyłana jest informacja, że uwierzytelnienie przebiegło poprawnie. Jeżeli zaś danego konta nie znaleziono, lub znaleziono, ale hasło się nie zgadza zwracany jest błąd. Błąd ten już w samym mechanizmie weryfikacji powinien być sformułowany ogólnie, nie powinno się odsyłać informacji pozwalających ewentualnemu atakującemu dowiedzieć się, że dany login istnieje, ale niech spróbuje z innym hasłem.

Jak dokładnie wygląda ten mechanizm weryfikacji? Lista kont, czyli nazwy użytkowników, ich uprawnienia, hasła, ewentualnie inne dane typu historia działań, wiadomości, ustawienia, etc. są trzymane albo w zwykłym pliku tekstowym, albo w bazie danych. Jest to najściślejsza tajemnica systemu. Hasła muszą być trzymane z postaci zaszyfrowanej. To niestety nie zawsze się zdarza. Co do zasady: nikt poza użytkownikiem nie może znać hasła. Ale nie chodzi tu tylko o mało prawdopodobną sytuację, kiedy to admin loguje się użytkownikiem i wyczynia na jego notabene konto niecne brewerie. Wiadomo, że hasła są, albo przynajmniej powinny być najbardziej strzeżoną częścią systemu. Nie ma jednak systemu odpornego na atak, nie można zakładać, że takowy istnieje. Trzeba brać pod uwagę prawdopodobieństwo, że wcześniej czy później każda baza z danymi użytkowników wpadnie w ręce bandytów, którzy z takich czy innych powodów ją upublicznią. Co wtedy?

Jeżeli hasła są trzymane w postaci jawnej system jest w całości przejęty. To oznacza, lub powinno oznaczać kompletną kompromitację i bankructwo organizacji mającej ten system. Sytuacja jest zupełnie inna jeśli hasła są zaszyfrowane. Bandyci wciąż nie są bez szans. Mogą wykupić moc obliczeniową w chmurze i hashcatem przelecieć bazę haseł. Jeżeli twórcy systemu pomyśleli, ale nie do końca i zastosowali jakiś prosty algorytm szyfrujący typu MD5, a użytkownicy stosowali hasła typu '12345sierpien' (sierpien, bo co miesiąc muszą zmieniać hasło), to bardzo szybko i bardzo tanio bandyci osiągnęli ten sam pozytywny dla siebie rezultat.

Zgoła inaczej jest jeżeli twórcy systemu pomyśleli poprawnie i wdrożyli właściwy algorytm szyfrujący czyli bcrypt oraz założyli dobre limity na liczbę znaków, czyli minimum 15 znaków, a maksimum na tyle ile nie będzie problemem dla maszyny (bo szyfrowanie jest zasobożerne). Również tę sytuację zmienić mogą sami użytkownicy jeśli mają dobrze skonstruowane hasła o właściwej długości. Wtedy, nawet jeśli są zaszyfrowane MD5 będą problemem dla hashcata i bandyci zadowolą się, jak to się mówi "nisko wiszącymi owocami", czyli hasłami typu 'buziaczek12'. Trudne hasła są o wiele bardziej kosztowne do złamania zarówno w kategoriach czasu jak i pieniędzy.

Na pytanie co jest ważniejsze: poprawna architektura systemu weryfikacji haseł, czy dobre hasła użytkowników odpowiem pytaniem: a co jest łatwiejsze, raz porządnie napisać program i zastosować właściwy algorytm, czy przekonać / zmusić użytkowników do właściwej polityki haseł?
