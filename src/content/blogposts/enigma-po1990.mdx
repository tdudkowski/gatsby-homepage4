...
Używanie menedżera haseł na przykładzie programu KeePassXC (darmowy i otwarty fork Keepass). Założenie: potrzebujemy menedżera haseł dla notebooka, na którym są dwa systemy oepracyjne (Windows i Ubuntu) i smarfona (Android). Zresztą niezależnie dla ilu komputerów i środowisk konfigurujemy manadżera, wygląda to tak samo. Użyjemy współdzielenia bazy danych przez Dropboxa oraz archiwizacji. Postaramy się to zrobić łagodnie, zaczynając od kluczowych elemntów systemu i niezbyt istotnych kont. Po kolei:
...
- Instalacja programu:
  - w Ubuntu `# apt install keepassxc` (jest to niestety w momencie, w którym to piszę dość stara wersja). Trzeba pamiętać, że oprogrammowanie związane z kryptografią i ogólnie z bezpieczeństwem musi być aktualizowane w pierwszej kolejności.
...
- Przy pierwszym wejściu na zdefiniowaną stronę z logowaniem zostaniemy zapytani o skojarzenie. I to naprawdę wszystko jeżeli chodzi o jedno srodowisko i utworzenie bazy danych, z której będą korzystały drugi system operacyjny oraz smartfon.
