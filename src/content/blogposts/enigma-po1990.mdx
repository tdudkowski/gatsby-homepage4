...
Używanie menedżera haseł na przykładzie programu KeePassXC. Założenie: potrzebujemy menedżera haseł dla notebooka, na którym są dwa systemy oepracyjne (Windows i Ubuntu) i smarfona (Android). Współdzielenie bazy danych przez Dropboxa, oraz archiwizacja. postaramy się to zrobić łagodnie, zaczynając od kluczowych elemntów systemu i niezbyt istotnych kont. Po kolei:

- Inwentaryzacja wszystkich założonych przez nas kont, łącznie z hasłami. Wszystkich założonych, nie tylko obecnie rzeczywiście używanych. Oceniamy, które są ważne (związane z finansami, tożsamością, uwierzytelniają nas na innych serwisach). Wbrew pozorom jest to długi proses i po latach aktywności w internecie trudny do skompletowania.
- Instalacja programu: w Ubuntu `# apt install keepassxc` (jest to niestety w momencie, w którym to piszę dość stara wersja), w Androidzie KeePassDX ze sklepu Google. Trzeba pamiętać, że oprogrammowanie związane z kryptografią i ogólnie z bezpieczeństwem musi być aktualizowane w pierwszje kolejności.
- Zakładając, że będziemy uzywać synchronizacji pomiędzy urządzeniami jest to ostatni moment na jej utworzenie. W podlinkowanym artykule Niebezpiecznika mowa jest o trzech możliwościach: iCloud, Google Drive i Dropbox. Te dwa ostatnie są dostępne jednocześnie dla wszystkich trzech najważniejszych systemów operacyjnych. [How To Sync Ubuntu To Your Google Drive](https://helpdeskgeek.com/linux-tips/how-to-sync-ubuntu-to-your-google-drive/), [Zainstaluj Dropbox, aby w pełni wykorzystać swoje konto](https://www.dropbox.com/install-linux)
...
- W programie tworzymy wpis, trzeba podać rozpoznawalną unikalną nazwę (tytuł) wpisu, nazwę użytkownika, hasło (można go utworzyć, podejrzeć) i url.
- Zaczynamy od desktopa, tutaj wszystkie (lub prawie wszystkie) konta używamy w przeglądarce WWW. Wybieramy jedną, którą będziemy do nich używać. Instalujemy rozszerzenie i w jego ustawieniach łączymy z bazą danych podając jej nazwę (identyfikator), program musi być w tym czasie włączony.
