...
- szyfrowanie danych end-to-end, zapewniające, że ewentualny podsłuchujący może odczytać tylko adres, z którym się łączymy, a nie przesyłane treści (adresy DNS, z którymi się łączymy można ukryć, używając Tora, VPN, lub DNS-over-HTTPS)
...
- [OnionShare](https://onionshare.org/) narzędzie do współdzielenie plików
- [Tails](https://tails.boum.org/index.en.html) bootowalny z USB system operacyjny intensywnie używający Tor-a, nastawiony na prywatność, polecany przez Edwarda Snowdena
- [Whonix - Software That Can Anonymize Everything You Do Online](https://www.whonix.org/) pakiet Tor-a na różne systemy operacyjne
- [Qubes OS](https://www.qubes-os.org/) system operacyjny używający wirtualizacji co pozwala na izolację programów i elementów systemu, co jest zabezpieczeniem przez 0-day, oraz uruchamianie programów windowsowych
...
Używanie menedżera haseł na przykładzie programu KeePassXC (darmowy i otwarty fork Keepass). Założenie: potrzebujemy menedżera haseł dla notebooka, na którym są dwa systemy operacyjne (Windows i Ubuntu) i smartfona (Android). Zresztą niezależnie dla ilu komputerów i środowisk konfigurujemy menadżera, wygląda to tak samo. Użyjemy współdzielenia bazy danych przez Dropboxa oraz archiwizacji. Postaramy się to zrobić łagodnie, zaczynając od kluczowych elementów systemu i niezbyt istotnych kont. Po kolei:

- Inwentaryzacja wszystkich założonych przez nas kont, łącznie z hasłami. Wszystkich założonych, nie tylko obecnie rzeczywiście używanych. Oceniamy, które są ważne (związane z finansami, tożsamością, uwierzytelniają nas na innych serwisach). Wbrew pozorom jest to długi proces i po latach aktywności w internecie trudny do skompletowania. Na początek wystarczą najważniejsze i ostatnio regularnie używane przez nas konta.
- Instalacja programu:
  - w Ubuntu `# apt install keepassxc` (jest to niestety w momencie, w którym to piszę dość stara wersja). Trzeba pamiętać, że oprogramowanie związane z kryptografią i ogólnie z bezpieczeństwem musi być aktualizowane w pierwszej kolejności.
  - w Androidzie KeePassDX ze sklepu Google.
- Zakładając, że będziemy używać synchronizacji pomiędzy urządzeniami jest to ostatni moment na jej utworzenie. W podlinkowanym artykule Niebezpiecznika mowa jest o trzech możliwościach: iCloud, Google Drive i Dropbox. Te dwa ostatnie są dostępne jednocześnie dla wszystkich trzech najważniejszych systemów operacyjnych.
...
- Uruchamiamy program. Na początek tworzymy bazę danych, w której będą trzymane dane logowania. Trzeba ustalić jej hasło główne i lokalizację pliku w synchronizowanym katalogu.
...
- Zaczynamy od desktopa, tutaj wszystkie (lub prawie wszystkie) konta używamy w przeglądarce WWW. Wybieramy tę, którą będziemy do nich używać. Instalujemy rozszerzenie i w jego ustawieniach łączymy z bazą danych podając jej nazwę (identyfikator), program musi być w tym czasie włączony.
- Przy pierwszym wejściu na zdefiniowaną stronę z logowaniem zostaniemy zapytani o skojarzenie. I to naprawdę wszystko jeżeli chodzi o jedno środowisko i utworzenie bazy danych, z której będą korzystały drugi system operacyjny oraz smartfon.
- Na smartfonie instalujemy program KeePassDX, importujemy bazę z Dropboxa (trzeba wybrać opcję Remote). Za każdym użyciem programu uzyskujemy do niej dostęp po wpisaniu hasła głównego. Kiedy ją widzimy to można w Ustawienia / Wypełnianie formularzy włączyć "Ustaw domyślną usługę autouzupełniania". Wtedy możemy włączyć już przeglądarkę na telefonie i uzyskamy opcję automatycznego zalogowania się.
...
Jeszcze jedną możliwością zarządzania hasłami jest trzymanie ich w przeglądarce: [Menedżer haseł | passwords.google.com](https://passwords.google.com/), Firefox Lockwise został wyłączony.
...
- Kacper Szurek ["Jak działa DNS over HTTPS (DOH)" [YT 14:44]](https://www.youtube.com/watch?v=WbDwcns24l4)
