...
Najpopularniejsza usługa analityczna, darmowa i dobra. Ostatnio Google porzuciło dawną wersję na rzecz Google Analytics 4. Jest to poważna zmiana i wymaga zmiany zarówno konfiguracji na stronie usługi jak i samego pluginu. Jeżeli nie potrzebujemy danych historycznych najlepiej dotychczasową usługę usunąć, utworzyć ją na nowo, stary plugin odinstalować i usunąć jego konfigurację z gatsby-config.js. Można też w razie potrzeby włączyć istniejącą dotąd usługę w skład nowej.

Niezależnie od tego jakiego pluginu używaliśmy do tej pory, dla Google Analytics 4 jedynym właściwym pluginem jest gatsby-plugin-google-tagmanager. Wszystkie inne pluginy tej usługi należy usunąć, a ten zainstalować. Szczegółowa instrukcja w podlinkowanym poniżej filmie. Jego autor wyjaśnia również konfigurację dołączającą dotychczas używaną usługę do Google Analytics 4.

W najprostszej konfiguracji wygląda to tak:
...
Pliku .env co do zasady nie wysyłamy na GitHuba ani inne publiczne miejsce. Musi zostać dodany do .gitignore. Trzeba doinstalować dotenv. Linkujemy do niego na samej górze gatsby-config.js:

```js
require("dotenv").config({ path: `.env`, })
```

Wszyscy dostawcy usług online umożliwiają wpisanie zmiennych środowiskowych w ustawieniach strony / aplikacji (np. Netlify: Site settings / Build & deploy / Environment variables). Tutaj pamiętajmy, że:
- dodajemy identyfikator tagu a nie usługi GA
- w konfiguracji parametrów środowiskowych usługi wpisujemy go w cudzysłowie
- adblocker, jak sama nazwa wskazuje, blokuje także usługę śledzenia przez GA4 (w każdym razie jedyny adblocker jaki ma sens, czyli [uBlock Origin](https://chrome.google.com/webstore/detail/ublock-origin/cjpalhdlnbpafiamejdnhcphjbkeiagm?hl=en))
- gatsby-plugin-google-tagmanager działa tylko dla Gatsby v4 w górę, trójka niestety musi pozostać przy starej usłudze Google Analytics, która zostanie zamknięta w lipcu 2023

Przeglądanie lokalnego builda dodaje się do statystyk.

- Guiding Digital ["How to Add Google Analytics 4 to a Gatsby Site Using Google Tag Manager" [YT 17:09]](https://www.youtube.com/watch?v=wX9euOw4mGY)
