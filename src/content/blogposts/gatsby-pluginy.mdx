...
Inaczej niż jest to zaprezentowane na filmie proponuję od razu zastosować wpisanie identyfikatora to pliku .env, którego co do zasady nie wysyłamy na GitHuba ani żadne inne publiczne miejsce. Musi zostać dodany do .gitignore. Trzeba doinstalować dotenv. Linkujemy do niego na samej górze gatsby-config.js:

```js
require("dotenv").config({ path: `.env`, })
```

Wszyscy dostawcy usług online umożliwiają wpisanie zmiennych środowiskowych w ustawieniach strony / aplikacji (np. Netlify: Site settings / Build & deploy / Environment variables).

Tutaj pamiętajmy, że:
- zmienną środowiskową jest identyfikator tagu (GTM) a nie usługi GA
